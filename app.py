...
if uploaded_file := st.file_uploader("Sheet số (123)", type=["json"]):
    ...
    <style>
        ...
        td {
            ...
            border-left: none;
            /* Dùng currentColor để tự bắt màu từ body */
            color: currentColor;
        }
        @media print {
    ...
    <script>
        function adjustTheme() {
            // Kiểm tra xem trình duyệt đang dùng theme nào
            const isDarkMode = window.matchMedia('(prefers-color-scheme: dark)').matches;
            const targetColor = isDarkMode ? '#FFFFFF' : '#000000';

            // Ép màu trực tiếp vào body
            document.body.style.color = targetColor;
            document.body.style.backgroundColor = 'transparent';

            // Đảm bảo tất cả các thẻ td cũng nhận đúng màu này
